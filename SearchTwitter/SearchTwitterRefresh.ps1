...
New-Window @windowAttributes -Show -On_Loaded {
    Register-PowerShellCommand -Run -In "0:1:0" -ScriptBlock {
        $targetLayout.Children | % {            
            $_.Children[1].DataContext = Search-Twitter $_.Tag
        }
    }
    ...
    ql PowerShell ShowUI dfinke jamesbru  jaykul | 
        ForEach { 
            $targetLayout.Children.Add( (New-TwitterFeed $_) ) 
        }
        
} {

    function Global:ql {$args}
    function Global:Search-Twitter {
        param ( 
    ...
    function Script:New-TwitterFeed ($search) {
